...
namespace FleeAndCatch_App.pages.content
{
    public partial class Control : ContentPage
    {
        ...
        {
            if (Communication.Client.Connected)
            {
                ...
                for (var i = 0; i < Enum.GetNames(typeof(RobotType.Type)).Length; i++)
                {
                    ...
                    {
                        if(t.Identification.Type == Enum.GetNames(typeof(RobotType.Type))[i])
        ...
        {
            await Navigation.PushAsync(new control.Control(null));
            //Robot is tapped, show detail page for start control
        }
    }
