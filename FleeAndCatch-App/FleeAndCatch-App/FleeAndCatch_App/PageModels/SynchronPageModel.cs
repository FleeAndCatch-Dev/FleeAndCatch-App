...
namespace FleeAndCatch_App.PageModels
{
    ...
    public class SynchronPageModel : FreshMvvm.FreshBasePageModel
    {
        ...
        private bool NewControlCmd()
        {
            ...
            if (SzenarioController.Changed)
            {
                foreach (var t in RobotController.Robots)
                {
                    ...
                    {
                        if(_robots[j].Identification.Id != t.Identification.Id) continue;
                        Robots[j] = new RobotModel(t);
                        break;
                    }
                }
            ...
            if (!SzenarioController.Refresh)
            {
                ...
                foreach (var t in _szenario.Robots)
                    t.Active = false;
